

<style>
    .status-select {
        padding: 6px 10px;
        border-radius: 15px;
        border: none;
        font-weight: bold;
        cursor: pointer;
        color: #fff;
        text-align: center;
        width: 100%;
        max-width: 130px;
        font-size: 12px;
        appearance: none;
        -webkit-appearance: none;
        text-align-last: center;
        transition: 0.2s;
        box-shadow: 0 2px 5px rgba(0,0,0,0.2);
    }
    .status-Pendente { background-color: #f39c12; }
    .status-EmAndamento { background-color: #3498db; }
    .status-Aguardando { background-color: #9b59b6; }
    .status-Concluido { background-color: #2ecc71; color: #000; }
    .status-Cancelado { background-color: #e74c3c; }
    .status-select:hover { opacity: 0.9; transform: scale(1.02); }
</style>

<div class="box-relatorio">
    <div class="page-header">
        <h1 class="login-title"><i class="bi bi-tools action-icon"></i>Ordens de Serviço</h1>
        <a href="index.php?rota=nova_os" class="btn-green text-decoration-none">
            <i class="bi bi-plus-circle action-icon"></i>Nova O.S.
        </a>
    </div>

    <div class="table-shell">
        <table>
            <thead>
                <tr>
                    <th>ID</th>
                    <th>Data</th>
                    <th>Empresa / Prestador</th>
                    <th>Resumo</th>
                    <th>Total</th>
                    <th class="text-center">Status</th>
                    <th>Ações</th>
                </tr>
            </thead>
            <tbody>
                <?php if (!empty($servicos)): ?>
                    <?php foreach($servicos as $os): ?>
                    <?php $classeStatus = str_replace(' ', '', $os['status'] ?? 'Pendente'); ?>
                    <tr>
                        <td class="text-muted">#<?= $os['id'] ?></td>
                        <td><?= !empty($os['data_abertura']) ? date('d/m/Y', strtotime($os['data_abertura'])) : '-' ?></td>
                        <td>
                            <small class="text-muted d-block">De: <?= htmlspecialchars($os['empresa'] ?? '') ?></small>
                            <strong style="color: var(--primary-color);">Para: <?= htmlspecialchars($os['prestador'] ?? '') ?></strong>
                        </td>
                        <td class="text-muted"><?= htmlspecialchars(substr($os['descricao'], 0, 30)) ?>...</td>
                        <td class="fw-bold" style="color: var(--brand-green);">R$ <?= number_format($os['valor_total'] ?? 0, 2, ',', '.') ?></td>
                        <td class="text-center">
                            <select class="status-select status-<?= $classeStatus ?>"
                                    onchange="mudarStatusOS(this, <?= $os['id'] ?>)">
                                <option value="Pendente" <?= $os['status'] == 'Pendente' ? 'selected' : '' ?>>Pendente</option>
                                <option value="Em Andamento" <?= $os['status'] == 'Em Andamento' ? 'selected' : '' ?>>Em Andamento</option>
                                <option value="Aguardando" <?= $os['status'] == 'Aguardando' ? 'selected' : '' ?>>Aguardando Peça</option>
                                <option value="Concluido" <?= $os['status'] == 'Concluido' ? 'selected' : '' ?>>Concluído</option>
                                <option value="Cancelado" <?= $os['status'] == 'Cancelado' ? 'selected' : '' ?>>Cancelado</option>
                            </select>
                        </td>
                        <td>
                            <?php if(isset($_SESSION['user_nivel']) && $_SESSION['user_nivel'] === 'admin'): ?>
                                <a href="index.php?rota=os_excluir&id=<?= $os['id'] ?>"
                                   onclick="return confirm('Tem certeza?')"
                                   class="text-decoration-none" style="color: var(--brand-red);">
                                    <i class="bi bi-trash3" style="font-size:18px;"></i>
                                </a>
                            <?php endif; ?>
                        </td>
                    </tr>
                    <?php endforeach; ?>
                <?php else: ?>
                    <tr><td colspan="7" class="text-center p-4">Nenhuma O.S. encontrada.</td></tr>
                <?php endif; ?>
            </tbody>
        </table>
    </div>
</div>

<script>
function mudarStatusOS(select, id) {
    var novoStatus = select.value;
    var classeCor = novoStatus.replace(/\s+/g, '');
    select.className = 'status-select status-' + classeCor;
    window.location.href = 'index.php?rota=os_mudar_status&id=' + id + '&status=' + novoStatus;
}
</script>