...
<head>
    ...
    <style>
        ...
        * { box-sizing: border-box; -webkit-print-color-adjust: exact !important; print-color-adjust: exact !important; } 
        body { font-family: 'Segoe UI', Arial, sans-serif; background: #555; margin: 0; padding: 20px; }
        
        /* ESTILO DA FOLHA A4 */
        .folha-a4 {
            background: #fff; width: 210mm; margin: 0 auto 20px auto;
            padding: 15mm; box-shadow: 0 0 10px rgba(0,0,0,0.5); color: #000;
            position: relative;
            page-break-inside: avoid; /* Evita quebras dentro da ficha */
        }
        
        /* QUEBRA DE PÁGINA NA IMPRESSÃO */
        .quebra-pagina { page-break-after: always; }

        h1 { text-align: center; border-bottom: 2px solid #000; padding-bottom: 10px; margin: 0 0 20px 0; font-size: 24px; }
        .campo { margin-bottom: 8px; font-size: 14px; border-bottom: 1px dotted #ccc; padding-bottom: 4px; }
        .campo strong { font-weight: bold; font-size: 15px; margin-right: 5px; }
        
        .grid-topo { display: grid; grid-template-columns: 2fr 1fr 1fr; gap: 15px; margin-bottom: 10px; }
        .grid { display: grid; grid-template-columns: 1fr 1fr; gap: 20px; }
        
        /* BOX DO PRODUTO (MODELO, COR E GRADE) */
        .box-produto { border: 2px solid #000; padding: 12px; margin: 15px 0; background: #f4f4f4; }
        
        /* CONTAINER DO MOCKUP (SUPORTA IMG E PDF) */
        .box-imagem {
            border: 2px solid #000; margin: 15px 0;
            display: flex; align-items: center; justify-content: center;
            background: #f9f9f9;
        }

        .aviso { font-weight: bold; background: #eee; padding: 10px; border: 1px solid #000; margin-top: 15px; font-size: 12px; }

        /* AJUSTES PARA IMPRESSORA */
        @media print {
            body { background: #fff; padding: 0; margin: 0; }
            .folha-a4 { box-shadow: none; width: 100%; margin: 0; padding: 10mm; border: none; }
...
<body>
    ...
    <div class="folha-a4 <?= ($index < count($lista_fichas) - 1) ? 'quebra-pagina' : '' ?>">
        ...
        <div class="box-produto">
            ...
            <div style="display:grid; grid-template-columns: 2.5fr 2fr 1fr; gap: 10px; align-items: center;">
                
                <div style="border-right: 1px dashed #999; padding-right:10px;">
                    <span style="font-size: 11px; text-transform: uppercase; color:#555;">Modelo / Produto:</span><br>
                    <strong style="font-size: 16px; text-transform: uppercase; display:block; margin-bottom:8px;">
                        <?= htmlspecialchars($ficha['modelo'] ?? '') ?>
                    </strong>
                    <span style="font-size: 11px; text-transform: uppercase; color:#555;">Cor:</span><br>
                    <strong style="font-size: 15px; text-transform: uppercase;">
                        <?= htmlspecialchars($ficha['cor'] ?? 'Padrão') ?>
                    </strong>
                </div>

                <div style="text-align:center; border-right: 1px dashed #999; padding-right:10px;">
                    <span style="font-size: 11px; text-transform: uppercase; color:#555;">Distribuição (Grade):</span><br>
                    <?php if(!empty($grade) && is_array($grade)): ?>
                        <table style="width:100%; margin-top:5px; border-collapse:collapse; font-size:13px;">
                            <tr style="border-bottom:1px solid #ccc;">
                                <?php foreach($grade as $tam => $qtd): ?>
                                    <th style="padding:2px; color:#555; font-size:10px;"><?= $tam ?></th>
                                <?php endforeach; ?>
                            </tr>
                            <tr>
                                <?php foreach($grade as $tam => $qtd): ?>
                                    <td style="font-weight:bold; font-size:16px; padding:2px;"><?= $qtd ?></td>
                                <?php endforeach; ?>
                            </tr>
                        </table>
                    <?php else: ?>
                        <strong style="font-size: 24px; display:block;"><?= htmlspecialchars($ficha['tamanho'] ?? 'UN') ?></strong>
                    <?php endif; ?>
                </div>

                <div style="text-align:center;">
                    <span style="font-size: 11px; text-transform: uppercase; color:#555;">Qtd Item:</span><br>
                    <strong style="font-size: 32px;"><?= htmlspecialchars($ficha['quantidade'] ?? '0') ?></strong>
                </div>
            </div>
        </div>

        <div class="grid">
            <div class="campo"><strong>WhatsApp:</strong> <?= htmlspecialchars($ficha['contato'] ?? '-') ?></div>
            <div class="campo"><strong>Data Pedido:</strong> <?= !empty($ficha['data_pedido']) ? date('d/m/Y', strtotime($ficha['data_pedido'])) : '-' ?></div>
        </div>
        ...
        <div class="campo" style="background: #f4f4f4; padding: 8px 12px; border-left: 4px solid #3498db; margin-top: 10px;">
            <strong>Meio de Pagamento:</strong> <?= htmlspecialchars($ficha['meio_pagamento'] ?? 'Não informado') ?>
        </div>

        <div class="campo" style="margin-top:5px;">
            <strong>DATA DE ENTREGA:</strong> 
            <span style="font-size:18px; font-weight:bold; color:#d35400;">
                <?= !empty($ficha['data_entrega']) ? date('d/m/Y', strtotime($ficha['data_entrega'])) : 'A DEFINIR' ?>
            </span>
        </div>

        <h3 style="margin-bottom:5px; margin-top:10px; font-size:14px; text-transform: uppercase;">Mockup / Referência Visual</h3>
        ...
        <?php if(!empty($ficha['observacoes'])): ?>
            <div class="campo" style="background:#fef9e7; padding:10px; border:1px solid #f39c12; font-size:13px;">
                <strong>Observações de Produção:</strong><br>
                <?= nl2br(htmlspecialchars($ficha['observacoes'])) ?>
            </div>
        <?php endif; ?>

        <div class="aviso">
            ⚠️ <strong>INSTRUÇÕES:</strong><br>
            • Verifique se a camiseta separada condiz com a <strong>COR</strong> e <strong>MODELO</strong> acima.<br>
            • Confira a grade de tamanhos antes de iniciar a estampagem.<br>
            • Em caso de dúvidas, consulte o pedido <strong>#<?= htmlspecialchars($ficha['numero_pedido'] ?? '') ?></strong> na plataforma <strong><?= htmlspecialchars($ficha['plataforma'] ?? '') ?></strong>.
        </div>
    </div>
